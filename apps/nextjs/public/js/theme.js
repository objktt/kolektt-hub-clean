...
var cx = 0, cy = 0, rx = 0, ry = 0, rafId = null; 
var bounds;
...
function onEnter(){ bounds = $wrapper.get(0).getBoundingClientRect(); }
function onMove(e){
if(!bounds) onEnter();
...
$orbItems.each(function(){
  ...
  this.style.transform = 'translate3d('+tx+'px,'+ty+'px,0) rotateY('+orbYRot+'deg)';
});
}
...
$(document).ready(function() {
  ...
  if (square) {
    ...
    function updateSquarePosition() {
      ...
      var currentX = (windowWidth * 0.90) - (scrollProgress * (windowWidth * 0.8));
