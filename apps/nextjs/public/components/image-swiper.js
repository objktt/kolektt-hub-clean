...
class ImageSwiper {
  constructor(options = {}) {
    this.images = options.images ? options.images.split(',').map(img => img.trim()).filter(img => img) : [];
    ...
    this.cardRotations = Array.from({ length: this.images.length }, () => 
  ...
  renderCards() {
    return this.cardOrder.map((originalIndex, displayIndex) => `
  ...
  getDurationFromCSS(variableName) {
    const value = getComputedStyle(this.container)?.getPropertyValue(variableName)?.trim();
  ...
  destroy() {
    ...
    if (this.container && this.eventHandlers) {
      const cardStack = this.container.querySelector('.card-stack');
      if (cardStack) {
        cardStack.removeEventListener('pointerdown', this.eventHandlers.handlePointerDown);
        cardStack.removeEventListener('pointermove', this.eventHandlers.handlePointerMove);
        cardStack.removeEventListener('pointerup', this.eventHandlers.handlePointerUp);
        cardStack.removeEventListener('pointerleave', this.eventHandlers.handlePointerUp);
      }
    }
  }
}
